  ...
  app.post("/api/admin/login", async (req, res) => {
    try {
      ...
      if (!adminPassword) {
        ...
        if (!envPassword) {
          return res.status(503).json({ success: false, message: "Admin password not configured. Set ADMIN_PASSWORD environment variable." });
        }
        if (password === envPassword) {
          // First login: hash and store the env password so it's used from DB going forward
    ...
    try {
      ...
      if (process.env.RESEND_API_KEY) {
        ...
        const { data, error } = await resendClient.emails.send({
          from: 'ERGO Dokumente <dokumente@anfrage.ergo-stuebe.de>',
  ...
  app.post("/api/callback/submit", async (req, res) => {
    try {
      ...
      if (process.env.RESEND_API_KEY) {
        ...
        const resendClient = new Resend(process.env.RESEND_API_KEY);
        const { data, error } = await resendClient.emails.send({
          from: 'ERGO Rückruf <rueckruf@anfrage.ergo-stuebe.de>',
          to: 'morino.stuebe@ergo.de',
